...
describe("User POST", (): void => {
    it("Creates account (201)", async (): Promise<void> => {
        ...
        const findNewUser = await User.findOne({ _id: postReq.body._id }).lean().exec();
...
describe("User PUT", (): void => {
    it("Edits basic user info (200)", async (): Promise<void> => {
        ...
        const findEditedUser = await User.findOne({ _id: postReq.body.content._id }).lean().exec();
    ...
    it("Edits user password (200)", async (): Promise<void> => {
        ...
        const editedUser: UserInterface | null = await User.findOne({ first_name: 'Zane', last_name: 'Zaneson' }).lean().exec();
    ...
    it("User verified (200)", async (): Promise<void> => {
        ...
        const verifiedUser: UserInterface | null = await User.findOne({ first_name: 'Zane', last_name: 'Zaneson' }).exec();
