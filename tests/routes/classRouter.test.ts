...
describe('Class GET ', (): void => {
	it('Queries all classes (200)', async (): Promise<void> => {
		...
		const searchReq: Response = await request(app)
			.get(
				`/classes/search?name=${searchName}&subject=${searchSubject}&teacher=${searchTeacher}`
			)
...
describe('Class PUT', (): void => {
	...
	it('Add teacher to class', async (): Promise<void> => {
		...
		expect(
			newTeachArr?.some((t) => t.toString() === savedTeacher._id.toString())
		).toBeTruthy;
	});
	it('Removes teacher from class', async (): Promise<void> => {
		...
		const newTeachArr: ObjectId[] | undefined = editedClass?.teachers;
		expect(newTeachArr?.some((t) => t.toString() === teacherId?.toString()))
			.toBeFalsy;
	});
});
...
describe('Class DELETE', (): void => {
	it('Deletes class.', async (): Promise<void> => {
		...
		const deletedClass: ClassInterface | null = await ClassModel.findOne({
			_id: targetId,
		})
			.lean()
			.exec();
		const searchStudents: StudentInterface[] | null = await studentModel
			.find({ classes: targetId })
			.lean()
			.exec();
