		...
		if (!errors.isEmpty()) {
			...
			if (!userExists) {
				...
				if (!match) {
					...
					jwt.sign({ user: req.body.email, userId: userExists._id, org: userExists.org, accType: userExists.accType }, secretKey, { expiresIn: req.query.stayLogged ? "9999 years" : "30 min" }, (err, token) => {
