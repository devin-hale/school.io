		...
		if (!errors.isEmpty()) {
			...
			try {
				if (!req.query) {
					...
					}).populate("teachers").populate("org").lean().exec()
	...
	asyncHandler(async (req, res, next): Promise<void> => {
		...
		if (!errors.isEmpty()) {
			...
			try {
				...
				if (classExists) {
					const deletedClass: Document | null = await ClassModel.findOneAndDelete({ _id: targetId }).exec();

					await studentModel.updateMany({ classes: targetId }, { $pull: { classes: targetId } }).exec();
...
export default { get_class_instance, get_org_classes, create_class, edit_class, add_teacher, remove_teacher, delete_class, search_class };
